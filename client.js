...
function addMarkerToMap(data) {
  ...
  if (markers.length == 0) {
    for (var i = 0; i < data.length; ++i) {
      ...
      newMarker.bindPopup('<h3 class="popup-header">' + data[i].name + ' [' + data[i].id + ']</h3><p class="popup-text">Box ID: ' + data[i].box_id + '</p><p class="popup-text">License plate: ' + data[i].number + '</p>');
      newMarker.addTo(map);
